...
async function callBridge(system: string, user: string, maxTokens = 14000): Promise<string> {
  ...
  try {
    ...
    // crashing. Surface a clear restart instruction instead of leaving the user
...
const SECTION_INTENTS: Record<SectionKind, string> = {
  ...
  cta: 'Soft pitch for the offer. Name the barrier (what implementing alone would cost in time/energy/uncertainty), name the shortcut (the offer), say what it includes, drop a casual invite. Not pushy. Usually mid-video or late-video, before the outro. Pulls from Solopreneur Systems context.',
